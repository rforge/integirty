...
function(platforms, platformsCtr, assayType=c('Expr', 'Methy', 'CN'), model=3, 
			guessing=FALSE, permutationMethod=NULL, fold=1, nPerm=200, echo=TRUE, parallel=FALSE, ...){
	...
	if(permutationMethod != 'none'){
		permutationMethod <- try(match.arg(permutationMethod, c('gene sampling', 'sample label permutation'), several.ok=FALSE), silent=TRUE)
	...
	dichotomizeFunc <- function(i) {
		if(assayType[i]=='Expr') {
		res <- dichotomize(platforms[[i]], platformsCtr[[i]], assayType=assayType[i], parallel=parallel) # parallel only happens on expression data
		} else {
