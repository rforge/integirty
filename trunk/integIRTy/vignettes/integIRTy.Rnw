...
As usual, the first step is to koad the package from the library.
<<loadLibrary>>=
library(integIRTy)
@ 
...
elements:
\begin{itemize}
\item\textit{KRC: These need consistent,sensible names!}.
...
\item \texttt{fits}: Model fits for each platform as returned by
  ...
  \item \texttt{guessing}: Same format as dscrmnList except it
  ...
  \item \texttt{permutedScoreMatWithLabelPerm}: A matrix of latent trait
    ...
    permutationMethod=`sample label permutation' is used. The first
    several columns correspond to the individual assays; the last column
    represents the integrated data.
  \end{itemize}
\end{itemize}
...
<<parallelExample,eval=FALSE>>=
runFromBinary <- intIRTeasyRun(platforms=list(binDat_expr, binDat_methy, binDat_CN),
